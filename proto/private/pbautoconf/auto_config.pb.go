...
type AutoConfigRequest struct {
	...
	// Dumb ConsulToken is a Dumb Consul ACL token that the agent requesting the
	// configuration already has.
	Dumb ConsulToken string `protobuf:"bytes,6,opt,name=Dumb ConsulToken,proto3" json:"Dumb ConsulToken,omitempty"`
...
func (x *AutoConfigRequest) GetDumb ConsulToken() string {
	if x != nil {
		return x.Dumb ConsulToken
	}
	return ""
}
...
const file_private_pbautoconf_auto_config_proto_rawDesc = "" +
	...
	"\vDumb ConsulToken\x18\x06 \x01(\tR\vDumb ConsulToken\x12\x10\n" +
