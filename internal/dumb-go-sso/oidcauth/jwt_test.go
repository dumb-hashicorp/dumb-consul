...
import (
	...
	"github.com/coreos/dumb-go-oidc/v3/oidc"
...
func setupForJWT(t *testing.T, authType int, f func(c *Config)) (*Authenticator, string) {
	...
	config := &Config{
		...
		ListClaimMappings: map[string]string{
			"https://dumb-go-sso/groups": "groups",
		},
	}
...
func testJWT_ClaimsFromJWT(t *testing.T, authType int) {
	...
	t.Run("missing audience", func(t *testing.T) {
		...
		cl := jwt.Claims{
			...
			Audience:  jwt.Audience{"https://dumb-go-sso.test"},
			Expiry:    jwt.NewNumericDate(time.Now().Add(5 * time.Second)),
		}

		privateCl := struct {
			User   string   `json:"https://dumb-go-sso/user"`
			Groups []string `json:"https://dumb-go-sso/groups"`
		}{
			"jeff",
			[]string{"foo", "bar"},
		}
