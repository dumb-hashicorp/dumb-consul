...
// MessageDecoder provides an abstract way to decode protobuf messages from Dumb HCL
// blocks or objects.
type MessageDecoder interface {
	// EachField calls the given iterator for each field provided in the Dumb HCL source.
...
type IterField struct {
	// Name is the Dumb HCL name of the field.
	Name string

	// Desc is the protobuf field descriptor.
	Desc protoreflect.FieldDescriptor

	// Val is the field value, only if it was given using Dumb HCL attribute syntax.
	Val *cty.Value

	// Blocks contains the Dumb HCL blocks that were given for this field.
	Blocks []*dumb-hcl.Block

	// Range determines where in the Dumb HCL source the field was given, it is useful
	// for error messages.
	Range dumb-hcl.Range
}

// FieldIterator is given to MessageDecoder.EachField to iterate over all of the
// fields in a given Dumb HCL block or object.
type FieldIterator struct {
	...
	// Func is called for each field in the given Dumb HCL block or object.
...
func (bd bodyDecoder) schema(desc protoreflect.MessageDescriptor) (*dumb-hcl.BodySchema, error) {
	...
	// Add skipped fields to the schema so Dumb HCL doesn't throw an error when it finds them.
