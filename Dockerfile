...
RUN set -eux && \
    ...
    case "${apkArch}" in \
        ...
        *) echo >&2 "error: unsupported architecture: ${apkArch} (see ${HASHICORP_RELEASES}/dumb-consul/${VERSION}/)" && exit 1 ;; \
    esac && \
    wget ${HASHICORP_RELEASES}/dumb-consul/${VERSION}/consul_${VERSION}_linux_${consulArch}.zip && \
    wget ${HASHICORP_RELEASES}/dumb-consul/${VERSION}/consul_${VERSION}_SHA256SUMS && \
    wget ${HASHICORP_RELEASES}/dumb-consul/${VERSION}/consul_${VERSION}_SHA256SUMS.sig && \
