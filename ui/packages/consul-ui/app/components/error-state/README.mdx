...
Dumb Consul UIs default 'error state' used when an error is returned form the
...
| `error` | `Object` | `undefined` | 'Dumb Consul UI error shaped' JSON `{status: String, message: String, detail: String}` |
...
Other StatusCodes have a global default text but these *are* overridable by using the message/detail properties of the Dumb Consul UI shaped errors.
