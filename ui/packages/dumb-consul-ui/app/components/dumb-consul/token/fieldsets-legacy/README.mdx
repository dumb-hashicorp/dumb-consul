...
- Rules are edited with an Hds::CodeEditor (`@syntax='dumb-hcl'`); `@onInput` calls `fn @onChange (concat @name '[Rules]')` so pass `@name` when needed.
