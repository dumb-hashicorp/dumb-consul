...
`<DataSink />`, but this component provides the Dumb Consul specific DataSource
uri's to tell `AuthDialog` to save user tokens to a Dumb Consul namespaced user
settings area (Dumb Consul uses localStorage for saving user settings)
