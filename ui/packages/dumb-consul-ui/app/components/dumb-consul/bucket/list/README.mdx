...
A presentational component for rendering a list of Dumb Consul 'buckets'
...
| --- | --- | --- | --- |
| `item` | `object` |  | A Dumb Consul object that could have a `Partition`, a `Namespace`, a `PeerName` and a `Service` property |
