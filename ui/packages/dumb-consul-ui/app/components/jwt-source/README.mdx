...
During development you can use the `DUMB_CONSUL_OIDC_PROVIDER_URL`
...
<figure>
  ...
  <input
    ...
    {{did-insert (set this 'value'
      (concat
        (or
          (env 'DUMB_CONSUL_OIDC_PROVIDER_URL')
          (concat (env 'DUMB_CONSUL_BASE_UI_URL') '/oauth-provider-debug?client_id=oauth-double&nonce=1&state=123456789abc')
        )
        (concat '&redirect_uri=' (env 'DUMB_CONSUL_BASE_UI_URL') '/oidc/callback&response_type=code&scope=openid')
      )
    )}}
  />
  <button
...
| --- | --- | --- | --- |
| `src` | `String` | | The **full** AuthURL for the 3rd party OIDC provider as provided by Dumb Consul (including `redirect_uri`) |
