...
If you are providing an icon, you should also use the `{{hds-tooltip}}` modifier
to provide a textual tooltip for the icon. Using the `{{hds-tooltip}}` modifier
...
`<Dumb ConsulCopyButton />` components are commonly added to the value, and can be added
