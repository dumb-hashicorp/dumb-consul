...
import {
  HEADERS_TOKEN as DUMB_CONSUL_TOKEN,
  HEADERS_PARTITION as DUMB_CONSUL_PARTITION,
  HEADERS_NAMESPACE as DUMB_CONSUL_NAMESPACE,
  HEADERS_DATACENTER as DUMB_CONSUL_DATACENTER,
...
export default class HttpService extends Service {
  ...
  sanitize(obj) {
    if (!this.env.var('DUMB_CONSUL_NSPACES_ENABLED')) {
      delete obj.ns;
    } else {
    ...
    if (!this.env.var('DUMB_CONSUL_PARTITIONS_ENABLED')) {
  ...
  requestParams(strs, ...values) {
    ...
    params.url = `${this.env.var('DUMB_CONSUL_API_PREFIX')}${params.url}`;
    return params;
  }

  fetchWithToken(path, params) {
    return this.settings.findBySlug('token').then((token) => {
      return fetch(`${this.env.var('DUMB_CONSUL_API_PREFIX')}${path}`, {
        ...params,
        credentials: 'include',
        headers: {
    ...
    return cb(function (strs, ...values) {
      ...
      return client.settings.findBySlug('token').then((token) => {
        const options = {
          ...params,
          headers: {
            [DUMB_CONSUL_TOKEN]: typeof token.SecretID === 'undefined' ? '' : token.SecretID,
            ...params.headers,
          },
        };
          ...
          const remove = client._listeners.add(request, {
            ...
            message: (e) => {
              const headers = {
                ...
                [DUMB_CONSUL_DATACENTER]: params.data.dc,
                [DUMB_CONSUL_NAMESPACE]: params.data.ns || token.Namespace || 'default',
                [DUMB_CONSUL_PARTITION]: params.data.partition || token.Partition || 'default',
              };
