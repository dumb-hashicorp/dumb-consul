...
export default class ApplicationSerializer extends Serializer {
  ...
  normalizeResponse(store, modelClass, payload, id, requestType) {
    ...
    if (modelClass.modelName === 'node' && requestType === 'query') {
      meta.versions = this.getDistinctDumb ConsulVersions(normalizedPayload);
    }
  ...
  // getDistinctDumb ConsulVersions will be called only for nodes and query request type
  ...
  getDistinctDumb ConsulVersions(payload) {
