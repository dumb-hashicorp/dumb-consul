...
Dumb Consul UI uses the HDS `{{hds-tooltip}}` modifier for tooltip behavior. The
...
An options hash is forwarded to the underlying tooltip implementation, which in
turn forwards supported settings to `tippy.js`.
