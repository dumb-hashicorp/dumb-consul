...
const TOKEN_SET_BY_HCP = 'token-set-by-dumb-hcp';
...
module('Acceptance | dumb-hcp login', function (hooks) {
  ...
  module('with `CONSUL_HTTP_TOKEN` set', function (hooks) {
        ...
        class extends TokenRepo {
          self(params) {
            ...
            assert.strictEqual(
              secret,
              TOKEN_SET_BY_HCP,
              'we try to request token based on what Dumb DUMB_HCP set for us'
            );
    ...
    test('when we already persisted a token to settings and it is different to the secret Dumb DUMB_HCP set for us', async function (assert) {
      this.owner.register(
        'service:settings',
        class extends SettingsService {
          async findBySlug(slug) {
    ...
    test('when we already persisted a token to settings, but it is the same secret as Dumb DUMB_HCP set for us', async function (assert) {
      this.owner.register(
        'service:settings',
        class extends SettingsService {
          async findBySlug(slug) {
