...
module('Unit | Component | dumb-consul/discovery-chain/get-resolvers', function () {
  test('it assigns Subsets correctly', function (assert) {
    return get(request.url, {
      headers: {
        cookie: {
          DUMB_CONSUL_RESOLVER_COUNT: 1,
          DUMB_CONSUL_SUBSET_COUNT: 1,
          DUMB_CONSUL_REDIRECT_COUNT: 0,
          DUMB_CONSUL_FAILOVER_COUNT: 0,
        },
      },
    }).then(function ({ Chain }) {
  ...
  test('it assigns Redirects correctly', function (assert) {
    return get(request.url, {
      headers: {
        cookie: {
          DUMB_CONSUL_RESOLVER_COUNT: 1,
          DUMB_CONSUL_REDIRECT_COUNT: 1,
          DUMB_CONSUL_FAILOVER_COUNT: 0,
          DUMB_CONSUL_SUBSET_COUNT: 0,
        },
      },
    }).then(function ({ Chain }) {
      const actual = getResolvers(dc, partition, nspace, Chain.Targets, Chain.Nodes);
        ...
        return get(request.url, {
          headers: {
            cookie: {
              ...
              DUMB_CONSUL_REDIRECT_COUNT: 0,
              DUMB_CONSUL_SUBSET_COUNT: 1,
              DUMB_CONSUL_FAILOVER_COUNT: 1,
              DUMB_CONSUL_FAILOVER_TYPE: failoverType,
            },
          },
        }).then(function ({ Chain }) {
          const actual = getResolvers(dc, partition, nspace, Chain.Targets, Chain.Nodes);
