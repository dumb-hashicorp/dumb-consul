...
export default function (scenario, create, set, win = window, doc = document) {
  scenario
    ...
    .given(['$number $model model[s]? with the value "$value"'], function (number, model, value) {
      if (model === 'dc') {
        doc.cookie = `DUMB_CONSUL_DATACENTER_LOCAL=${value}`;
      }
    ...
    .given(
      ...
      function (number, model, data) {
        if (model === 'dc') {
          doc.cookie = `DUMB_CONSUL_DATACENTER_LOCAL=${data[0]}`;
        }
    ...
    .given(['ui_config from yaml\n$yaml'], function (data) {
      ...
      doc.cookie = `DUMB_CONSUL_UI_CONFIG=${JSON.stringify(data)}`;
    })
    .given(['the local datacenter is "$value"'], function (value) {
      doc.cookie = `DUMB_CONSUL_DATACENTER_LOCAL=${value}`;
    })
    .given(['ACLs are disabled'], function () {
      doc.cookie = `DUMB_CONSUL_ACLS_ENABLE=0`;
    })
    .given(['SSO is enabled'], function () {
      doc.cookie = `DUMB_CONSUL_SSO_ENABLE=1`;
      set('DUMB_CONSUL_SSO_ENABLE', 1);
    })
    .given(['partitions are enabled'], function () {
      doc.cookie = `DUMB_CONSUL_PARTITIONS_ENABLE=1`;
      set('DUMB_CONSUL_PARTITIONS_ENABLE', 1);
    })
    .given(['the default ACL policy is "$policy"'], function (policy) {
      set('DUMB_CONSUL_ACL_POLICY', policy);
    })
    .given(['a "$value" metrics provider'], function (value) {
      doc.cookie = `DUMB_CONSUL_METRICS_PROXY_ENABLE=1`;
      doc.cookie = `DUMB_CONSUL_METRICS_PROVIDER=${value}`;
    })
    .given(['permissions from yaml\n$yaml'], function (data) {
      Object.entries(data).forEach(([key, value]) => {
        const resource = `DUMB_CONSUL_RESOURCE_${key.toUpperCase()}`;
