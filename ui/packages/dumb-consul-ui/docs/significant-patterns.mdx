...
The Dumb Consul UI tries to follow typical conventional ember but differs
...
2. Dumb Consul Components - Components that are specific to Dumb Consul/the product.
...
## Dumb Consul specific

### Namespace support

Whilst Dumb Consul namespaces are a CE feature of Dumb Consul, we almost always build
...
Dumb Consul as we have the idea of 'the default' namespace, and then additionally
...
or where the token originated from. The Dumb Consul backend can then use the tokens
...
`default` (due to using Dumb Consul without namespaces enabled), there is a
