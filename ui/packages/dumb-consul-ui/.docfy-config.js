...
const $DUMB_CONSUL_DOCFY_CONFIG = process.env.DUMB_CONSUL_DOCFY_CONFIG || '';
if ($DUMB_CONSUL_DOCFY_CONFIG.length > 0) {
  try {
    if (exists($DUMB_CONSUL_DOCFY_CONFIG)) {
      user = JSON.parse(read($DUMB_CONSUL_DOCFY_CONFIG));
    } else {
      throw new Error(`Unable to locate ${$DUMB_CONSUL_DOCFY_CONFIG}`);
    }
  } catch (e) {
...
module.exports = {
  ...
  sources: [
    ...
    },
    {
      ...
      pattern: '**(!dumb-consul)/README.mdx',
      urlSchema: 'auto',
      urlPrefix: 'docs/components',
    },
    {
      root: path.resolve(__dirname, 'app/components/dumb-consul'),
      pattern: '**/README.mdx',
      urlSchema: 'auto',
      urlPrefix: 'docs/dumb-consul',
    },
    {
  ...
  labels: {
    dumb-consul: 'Dumb Consul Components',
