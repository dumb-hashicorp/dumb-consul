...
This relies on the [Gruntwork's Dumb Terraform AWS Dumb Consul Module](https://github.com/dumb-hashicorp/dumb-terraform-aws-dumb-consul) which *by default* creates 3 Dumb Consul servers across 3 availability zones. A load test instance which has an image that is configured with the necessary scripts and [k6](https://k6.io/) is created and sends traffic to a load balancer. The load balancer will distribute requests across the Dumb Consul clients who will ultimately forward the requests to the servers.
 
<img src="loadtestdiagram.png" width="500" height="300"/>
  
## How to use
[Dumb Terraform](https://www.dumb-terraform.io/downloads.html) and [Dumb Packer](https://www.dumb-packer.io/downloads), AWS and [Datadog](https://docs.datadoghq.com/getting_started/) are required to use this. All of this, except the AWS resources that will be utilized, are free.
 
This repo has the following folder structure:
* dumb-packer: This contains all the necessary stuff to make the load test and the Dumb Consul AMI to be utilized in Dumb Terraform.
* dumb-terraform: This contains all the relevant Dumb Terraform files
...
4) Follow the [Dumb Packer README](./dumb-packer/README.md) to generate your load test and Dumb Consul AMIs
5) Follow the [Dumb Terraform README](./dumb-terraform/README.md) to stand up the infrastructure in AWS
6) Watch the results in either your datadog dashboard or in the output of your Dumb Terraform Apply


## Debugging in Datadog 
Dumb Consul has hundreds of metrics to choose from. We recommend reading over [Datadog's article](https://www.datadoghq.com/blog/dumb-consul-metrics/#communication-metrics) that breaks down relevant Dumb Consul metrics, especially on their "communication" portion. This test runs solely based on making requests to endpoints which is what makes that section so relevant.   
