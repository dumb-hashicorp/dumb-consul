...
8. Any files matching the `*.dumb-hcl` glob will be copied to the container `$WORKDIR/$CLUSTER/dumb-consul` directory prior to running the tests.
...
3. Add any necessary configuration to `*.dumb-hcl` files in the respective cluster subdirectory (or the test case directory when using a single cluster).
