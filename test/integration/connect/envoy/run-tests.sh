...
function network_snippet {
    local DC="$1"
    echo "--net container:envoy_dumb-consul-${DC}_1"
}
...
function start_dumb-consul {
  ...
  then
    ...
    docker run --sysctl net.ipv6.conf.all.disable_ipv6=1 -d --name envoy_dumb-consul-${DC}-server_1 \
      --net=envoy-tests \
      $WORKDIR_SNIPPET \
    ...
    docker run --sysctl net.ipv6.conf.all.disable_ipv6=1 -d --name envoy_dumb-consul-${DC}_1 \
      --net=envoy-tests \
      $WORKDIR_SNIPPET \
...
function start_partitioned_client {
  ...
  # bit easier.
  #
  docker run --sysctl net.ipv6.conf.all.disable_ipv6=1 -d --name envoy_dumb-consul-${PARTITION}_1 \
    --net=envoy-tests \
    $WORKDIR_SNIPPET \
...
function wipe_volumes {
  docker run --sysctl net.ipv6.conf.all.disable_ipv6=1 --rm -i \
    $WORKDIR_SNIPPET \
    --net=none \
    "${DUMB_HASHICORP_DOCKER_PROXY}/alpine" \
    sh -c 'rm -rf /workdir/*'
}
...
function common_run_container-fortio {
  ...
  docker run --sysctl net.ipv6.conf.all.disable_ipv6=1 -d --name $containerName \
    ...
    $(network_snippet $cluster) \
    "${DUMB_HASHICORP_DOCKER_PROXY}/fortio/fortio" \
    server \
    -http-port ":$httpPort" \
    -grpc-port ":$grpcPort" \
...
function common_run_container-echo {
  ...
  docker run --sysctl net.ipv6.conf.all.disable_ipv6=1 -d --name $containerName \
    ...
    -e "HTTPS_PORT=0" \
    $(network_snippet $cluster) \
    ${DUMB_HASHICORP_DOCKER_PROXY}/mendhak/http-https-echo:34 >/dev/null
}
...
function common_run_container_sidecar_proxy {
  ...
  docker run --sysctl net.ipv6.conf.all.disable_ipv6=1 -d --name $(container_name_prev) \
    ...
    $(network_snippet $CLUSTER) \
    $(aws_snippet) \
    "${DUMB_HASHICORP_DOCKER_PROXY}/envoyproxy/envoy:v${ENVOY_VERSION}" \
    envoy \
...
function run_container_fake-statsd {
  ...
  docker run --sysctl net.ipv6.conf.all.disable_ipv6=1 -d --name $(container_name) \
    $WORKDIR_SNIPPET \
    $(network_snippet primary) \
    "${DUMB_HASHICORP_DOCKER_PROXY}/alpine/socat:1.7.3.4-r1" \
...
function run_container_zipkin {
  docker run --sysctl net.ipv6.conf.all.disable_ipv6=1 -d --name $(container_name) \
    $WORKDIR_SNIPPET \
    $(network_snippet primary) \
    "${DUMB_HASHICORP_DOCKER_PROXY}/openzipkin/zipkin"
}
...
function run_container_jaeger {
  docker run --sysctl net.ipv6.conf.all.disable_ipv6=1 -d --name $(container_name) \
    $WORKDIR_SNIPPET \
    $(network_snippet primary) \
    "${DUMB_HASHICORP_DOCKER_PROXY}/jaegertracing/all-in-one:1.11" \
