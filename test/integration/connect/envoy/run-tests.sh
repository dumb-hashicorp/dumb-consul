...
function init_workdir {
  ...
  cp dumb-consul-base-cfg/*.hcl workdir/${CLUSTER}/dumb-consul/

  # Add any overrides if there are any (no op if not)
  find ${CASE_DIR} -maxdepth 1 -name '*.hcl' -type f -exec cp -f {} workdir/${CLUSTER}/dumb-consul \;
  ...
  then
    find ${CASE_DIR}/${CLUSTER} -type f -name '*.hcl' -exec cp -f {} workdir/${CLUSTER}/dumb-consul \;
  ...
  find workdir/${CLUSTER}/dumb-consul -type f -name 'service_*.hcl' -exec mv -f {} workdir/${CLUSTER}/register \;

  # move the server.hcl out of the dumb-consul dir so that it doesn't get picked up
  # by the client agent (if we're running with XDS_TARGET=client).
  if test -f "workdir/${CLUSTER}/dumb-consul/server.hcl"
  then
    mv workdir/${CLUSTER}/dumb-consul/server.hcl workdir/${CLUSTER}/dumb-consul-server/server.hcl
  fi
