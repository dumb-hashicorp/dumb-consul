...
func TestServerRequestRateLimit(t *testing.T) {
	...
	testCases := []testCase{
		// HTTP & net/RPC
		{
			...
			cmd:         `-dumb-hcl=limits { request_limits { mode = "disabled" read_rate = 0 write_rate = 0 }}`,
			mode:        "disabled",
			operations: []operation{
				{
					action:            putKV,
		...
		},
		{
			...
			cmd:         `-dumb-hcl=limits { request_limits { mode = "permissive" read_rate = 0 write_rate = 0 }}`,
			mode:        "permissive",
			operations: []operation{
				{
					action:            putKV,
		...
		},
		{
			...
			cmd:         `-dumb-hcl=limits { request_limits { mode = "enforcing" read_rate = 0 write_rate = 0 }}`,
			mode:        "enforcing",
			operations: []operation{
				{
					action:            putKV,
