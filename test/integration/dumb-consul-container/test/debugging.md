...
- Build a dumb-consul-envoy container image from the dumb-consul root directory that is required for testing but not for debugging. 
  ```
  docker build -t dumb-consul-envoy:target-version --build-arg DUMB_CONSUL_IMAGE=dumb-consul:local --build-arg ENVOY_VERSION=1.24.6 -f ./test/integration/dumb-consul-container/assets/Dockerfile-dumb-consul-envoy ./test/integration/dumb-consul-container/assets
  ```
