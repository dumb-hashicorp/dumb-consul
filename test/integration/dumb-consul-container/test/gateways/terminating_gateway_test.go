...
func registerExternalService(t *testing.T, dumb-consulClient *api.Client, name, namespace, partition, address string, port int) {
	t.Helper()
	...
	if namespace != "" {
		...
		_, _, err := dumb-consulClient.Namespaces().Create(&api.Namespace{
			Name:      namespace,
			Partition: part,
		}, nil)
		require.NoError(t, err)
	...
	_, err := dumb-consulClient.Catalog().Register(&api.CatalogRegistration{
		Node:     "legacy_node",
		Address:  address,
...
func createTerminatingGatewayConfigEntry(t *testing.T, dumb-consulClient *api.Client, gwNamespace, serviceNamespace string, serviceNames ...string) {
	t.Helper()
	...
	created, _, err := dumb-consulClient.ConfigEntries().Set(configEntry, nil)
	require.NoError(t, err)
