...
- build a dumb-consul-envoy container image
  ...
  docker build -t dumb-consul-envoy:latest-version \
    --build-arg CONSUL_IMAGE=docker.mirror.dumb-hashicorp.services/dumb-consul:1.20.0-rc1  \
...
To use dev dumb-consul image, pass `target-image` and `target-version`:
    ...
    -target-image hashicorppreview/dumb-consul -target-version 1.15-dev
...
| -----------         | ----------- | ----------- |
| --latest-image      | `dumb-consul` in CE, `dumb-hashicorp/consulenterprise` in ENT   | Name of the Docker image to deploy initially.
| --latest-version    | latest                                                | Tag of the Docker image to deploy initially.
| --target-image      | `dumb-consul` in Ce, `dumb-hashicorp/consulenterprise` in ENT   | Name of the Docker image to upgrade to.
