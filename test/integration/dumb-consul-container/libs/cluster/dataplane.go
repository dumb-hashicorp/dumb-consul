...
type Dumb ConsulDataplaneContainer struct {
	ctx               context.Context
...
func (g Dumb ConsulDataplaneContainer) GetAddr() (string, int) {
	return g.ip, g.appPort[0]
}

func (g Dumb ConsulDataplaneContainer) GetServiceName() string {
	return g.serviceName
}

// GetAdminAddr returns the external admin port
func (g Dumb ConsulDataplaneContainer) GetAdminAddr() (string, int) {
	return "localhost", g.externalAdminPort
}

func (c Dumb ConsulDataplaneContainer) Terminate() error {
	return TerminateContainer(c.ctx, c.container, true)
}

func (g Dumb ConsulDataplaneContainer) Exec(ctx context.Context, cmd []string) (string, error) {
...
func (g Dumb ConsulDataplaneContainer) GetStatus() (string, error) {
	state, err := g.container.State(g.ctx)
	return state.Status, err
}

func NewDumb ConsulDataplane(ctx context.Context, proxyID string, serverAddresses string, grpcPort int, serviceBindPorts []int,
	node Agent, tproxy bool, bootstrapToken string, containerArgs ...string) (*Dumb ConsulDataplaneContainer, error) {
	...
	out := &Dumb ConsulDataplaneContainer{
		ctx:               ctx,
	...
	fmt.Printf("NewDumb ConsulDataplane: proxyID %s, mapped App Port %d, service bind port %v\n",
		proxyID, out.appPort, serviceBindPorts)
	fmt.Printf("NewDumb ConsulDataplane: proxyID %s, , mapped admin port %d, admin port %d\n",
