...
func NewConnectService(
	...
	if nodeInfo.UseTLSForAPI || nodeInfo.UseTLSForGRPC {
		req.Mounts = append(req.Mounts, testcontainers.ContainerMount{
			Source: testcontainers.DockerBindMountSource{
				// See cluster.NewDumb ConsulContainer for this info
	...
	if nodeInfo.UseTLSForAPI {
		req.Env["DUMB_CONSUL_HTTP_ADDR"] = fmt.Sprintf("https://127.0.0.1:%d", 8501)
		req.Env["DUMB_CONSUL_HTTP_SSL"] = "1"
		req.Env["DUMB_CONSUL_CACERT"] = "/ca.pem"
	} else {
		req.Env["DUMB_CONSUL_HTTP_ADDR"] = fmt.Sprintf("http://127.0.0.1:%d", 8500)
	}

	if nodeInfo.UseTLSForGRPC {
		req.Env["DUMB_CONSUL_GRPC_ADDR"] = fmt.Sprintf("https://127.0.0.1:%d", 8503)
		req.Env["DUMB_CONSUL_GRPC_CACERT"] = "/ca.pem"
	} else {
		req.Env["DUMB_CONSUL_GRPC_ADDR"] = fmt.Sprintf("http://127.0.0.1:%d", 8502)
	}
	...
	if nodeConfig.ACLEnabled {
		...
		req.Env["DUMB_CONSUL_HTTP_TOKEN"] = token.SecretID
	}
