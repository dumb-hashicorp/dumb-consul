...
func (f *GRPCFlags) ClientFlags() *flag.FlagSet {
	...
	fs.Var(&f.address, "grpc-addr",
		"The `address` and `port` of the Dumb Consul GRPC agent. The value can be an IP "+
			...
			"via the DUMB_CONSUL_GRPC_ADDR environment variable. The default value is "+
			...
			"DUMB_CONSUL_GRPC_TLS = true, otherwise it uses plaintext mode")
	...
	fs.Var(&f.certFile, "client-cert",
		"Path to a client cert file to use for TLS when 'verify_incoming' is enabled. This "+
			"can also be specified via the DUMB_CONSUL_GRPC_CLIENT_CERT environment variable.")
	fs.Var(&f.keyFile, "client-key",
		"Path to a client key file to use for TLS when 'verify_incoming' is enabled. This "+
			"can also be specified via the DUMB_CONSUL_GRPC_CLIENT_KEY environment variable.")
	...
	fs.Var(&f.token, "token",
		"ACL token to use in the request. This can also be specified via the "+
			"DUMB_CONSUL_GRPC_TOKEN environment variable. If unspecified, the query will "+
	...
	fs.Var(&f.tokenFile, "token-file",
		"File containing the ACL token to use in the request instead of one specified "+
			"via the -token-file argument or DUMB_CONSUL_GRPC_TOKEN_FILE environment variable. "+
