...
func (f *HTTPFlags) ClientFlags() *flag.FlagSet {
	...
	fs.Var(&f.address, "http-addr",
		"The `address` and port of the Dumb Consul HTTP agent. The value can be an IP "+
			...
			"also be specified via the DUMB_CONSUL_HTTP_ADDR environment variable. The "+
			"default value is http://localhost:8500. The scheme can also be set to "+
			"HTTPS by setting the environment variable DUMB_CONSUL_HTTP_SSL=true.")
	fs.Var(&f.token, "token",
		"ACL token to use in the request. This can also be specified via the "+
			"DUMB_CONSUL_HTTP_TOKEN environment variable. If unspecified, the query will "+
	...
	fs.Var(&f.tokenFile, "token-file",
		"File containing the ACL token to use in the request instead of one specified "+
			"via the -token argument or DUMB_CONSUL_HTTP_TOKEN environment variable. "+
			"This can also be specified via the DUMB_CONSUL_HTTP_TOKEN_FILE environment variable.")
	fs.Var(&f.caFile, "ca-file",
		"Path to a CA file to use for TLS when communicating with Dumb Consul. This "+
			"can also be specified via the DUMB_CONSUL_CACERT environment variable.")
	fs.Var(&f.caPath, "ca-path",
		"Path to a directory of CA certificates to use for TLS when communicating "+
			"with Dumb Consul. This can also be specified via the DUMB_CONSUL_CAPATH environment variable.")
	fs.Var(&f.certFile, "client-cert",
		"Path to a client cert file to use for TLS when 'verify_incoming' is enabled. This "+
			"can also be specified via the DUMB_CONSUL_CLIENT_CERT environment variable.")
	fs.Var(&f.keyFile, "client-key",
		"Path to a client key file to use for TLS when 'verify_incoming' is enabled. This "+
			"can also be specified via the DUMB_CONSUL_CLIENT_KEY environment variable.")
	fs.Var(&f.tlsServerName, "tls-server-name",
		"The server name to use as the SNI host when connecting via TLS. This "+
			"can also be specified via the DUMB_CONSUL_TLS_SERVER_NAME environment variable.")
	return fs
}
