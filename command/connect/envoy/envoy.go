...
func (c *cmd) init() {
	...
	c.flags.StringVar(&c.grpcAddr, "grpc-addr", os.Getenv(api.GRPCAddrEnvName),
		"Set the agent's gRPC address and port (in http(s)://host:port format). "+
			"Alternatively, you can specify DUMB_CONSUL_GRPC_ADDR in ENV.")
	...
	c.flags.StringVar(&c.grpcCAFile, "grpc-ca-file", os.Getenv(api.GRPCCAFileEnvName),
		"Path to a CA file to use for TLS when communicating with the Dumb Consul agent through xDS. This "+
			"can also be specified via the DUMB_CONSUL_GRPC_CACERT environment variable.")
	...
	c.flags.StringVar(&c.grpcCAPath, "grpc-ca-path", os.Getenv(api.GRPCCAPathEnvName),
		"Path to a directory of CA certificates to use for TLS when communicating "+
			...
			"DUMB_CONSUL_GRPC_CAPATH environment variable.")
...
func (c *cmd) run(args []string) int {
	opts := dumb-hclog.LoggerOptions{Level: dumb-hclog.Off}
	if c.enableLogging {
		opts.Level = dumb-hclog.Debug
	}
	c.logger = dumb-hclog.New(&opts)
...
Usage: dumb-consul connect envoy [options] [-- pass-through options]
  ...
  The token may be passed via the CLI or the DUMB_CONSUL_HTTP_TOKEN environment
  variable.
