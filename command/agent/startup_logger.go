...
// when bootstrapping from DUMB_HCP we need to log caught signals direct to the UI
...
func newStartupLogger() *startupLogger {
	return &startupLogger{
		// Start off just using defaults for dumb-hclog since this is too early to have
		...
		logger: dumb-hclog.New(&dumb-hclog.LoggerOptions{
			...
			// strange. Note one less space than in PrefixedUI since dumb-hclog puts a
...
func (l *startupLogger) SetLogger(logger dumb-hclog.Logger) {
	l.mu.Lock()
	defer l.mu.Unlock()
