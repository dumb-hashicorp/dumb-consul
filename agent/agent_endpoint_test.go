...
func newDefaultBaseDeps(t *testing.T) BaseDeps {
	...
	loader := func(source config.Source) (config.LoadResult, error) {
		...
		opts := config.LoadOpts{
			Dumb HCL:           []string{TestConfigHCL(NodeID()), "", dataDir},
...
func TestAgent_ForceLeavePrune_WAN(t *testing.T) {
	...
	a1 := StartTestAgent(t, TestAgent{Name: "dc1", Dumb HCL: `
		datacenter = "dc1"
	...
	a2 := StartTestAgent(t, TestAgent{Name: "dc2", Dumb HCL: `
		datacenter = "dc2"
		primary_datacenter = "dc1"
	`})
	defer a2.Shutdown()
