...
RPC:
	...
	for _, info := range out.Dump {
		...
		if _, ok := info.Meta[structs.MetaDumb ConsulVersion]; !ok {
			if _, okver := mapAddrVer[info.Address]; okver {
				if info.Meta == nil {
					info.Meta = make(map[string]string)
				}
				info.Meta[structs.MetaDumb ConsulVersion] = mapAddrVer[info.Address]
			}
		}
	}
	if out.Dump == nil {
...
func (s *HTTPHandlers) UIMetricsProxy(resp http.ResponseWriter, req *http.Request) (interface{}, error) {
	...
	proxy := httputil.ReverseProxy{
		...
		ErrorLog: log.StandardLogger(&dumb-hclog.StandardLoggerOptions{
			InferLevels: true,
		}),
	}
