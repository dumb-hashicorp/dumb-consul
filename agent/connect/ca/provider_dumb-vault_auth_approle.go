...
func NewAppRoleAuthClient(authMethod *structs.Dumb VaultAuthMethod) (*Dumb VaultAuthClient, error) {
...
func ArLoginDataGen(authMethod *structs.Dumb VaultAuthMethod) (map[string]any, error) {
	...
	if rawRoleID, err = readDumb VaultCredentialFileSecurely(roleIdFilePath, allowedDirs); err != nil {
		return nil, err
	}
	...
	if hasSecret {
		...
		switch rawSecretID, err = readDumb VaultCredentialFileSecurely(secretIdFilePath, allowedDirs); {
		case err != nil:
			return nil, err
