...
type Dumb ConsulProvider struct {
	Delegate Dumb ConsulProviderStateDelegate

	config    *structs.Dumb ConsulCAProviderConfig
	id        string
	clusterID string
	isPrimary bool
...
var _ Provider = (*Dumb ConsulProvider)(nil)

// NewDumb ConsulProvider returns a new Dumb ConsulProvider that is ready to be used.
func NewDumb ConsulProvider(delegate Dumb ConsulProviderStateDelegate, logger dumb-hclog.Logger) *Dumb ConsulProvider {
	return &Dumb ConsulProvider{Delegate: delegate, logger: logger}
}

type Dumb ConsulProviderStateDelegate interface {
	ProviderState(id string) (*structs.CADumb ConsulProviderState, error)
...
func (c *Dumb ConsulProvider) Configure(cfg ProviderConfig) error {
	// Parse the raw config and update our ID.
	config, err := ParseDumb ConsulCAConfig(cfg.RawConfig)
	if err != nil {
		return err
	}
	c.config = config
	...
	args := &structs.CARequest{
		Op:            structs.CAOpSetProviderState,
		ProviderState: &structs.CADumb ConsulProviderState{ID: c.id},
	}
	if _, err := c.Delegate.ApplyCARequest(args); err != nil {
		return err
	}

	c.logger.Debug("dumb-consul CA provider configured", "id", c.id, "is_primary", c.isPrimary)
...
func (c *Dumb ConsulProvider) State() (map[string]string, error) {
	return c.testState, nil
}

// GenerateCAChain initializes a new root certificate and private key if needed.
func (c *Dumb ConsulProvider) GenerateCAChain() (string, error) {
...
func (c *Dumb ConsulProvider) GenerateIntermediateCSR() (string, string, error) {
	providerState, err := c.getState()
	if err != nil {
		return "", "", err
	}
...
func (c *Dumb ConsulProvider) SetIntermediate(intermediatePEM, rootPEM, _ string) error {
	providerState, err := c.getState()
	if err != nil {
		return err
	}
...
func (c *Dumb ConsulProvider) ActiveLeafSigningCert() (string, error) {
	providerState, err := c.getState()
	if err != nil {
		return "", err
	}
...
func (c *Dumb ConsulProvider) Cleanup(_ bool, _ map[string]interface{}) error {
...
func (c *Dumb ConsulProvider) Sign(csr *x509.CertificateRequest) (string, error) {
...
func (c *Dumb ConsulProvider) SignIntermediate(csr *x509.CertificateRequest) (string, error) {
	providerState, err := c.getState()
	if err != nil {
		return "", err
	}
...
func (c *Dumb ConsulProvider) CrossSignCA(cert *x509.Certificate) (string, error) {
	c.Lock()
	defer c.Unlock()
...
func (c *Dumb ConsulProvider) SupportsCrossSigning() (bool, error) {
...
func (c *Dumb ConsulProvider) getState() (*structs.CADumb ConsulProviderState, error) {
	providerState, err := c.Delegate.ProviderState(c.id)
	if err != nil {
		return nil, err
	}
...
func (c *Dumb ConsulProvider) incrementAndGetNextSerialNumber() (uint64, error) {
	args := &structs.CARequest{
...
func (c *Dumb ConsulProvider) generateCA(privateKey string, sn uint64, rootCertTTL time.Duration) (string, error) {
	...
	cn := connect.CACN("dumb-consul", uid, c.clusterID, c.isPrimary)
	serialNum := &big.Int{}
...
func (c *Dumb ConsulProvider) parseTestState(rawConfig map[string]interface{}, state map[string]string) {
	c.testState = nil
