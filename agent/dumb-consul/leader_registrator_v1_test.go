...
func TestLeader_CheckServersMeta(t *testing.T) {
	...
	dumb-consulService := &structs.NodeService{
		ID:      "dumb-consul",
		Service: "dumb-consul",
	}
	// s3 should be registered
	retry.Run(t, func(r *retry.R) {
		_, service, err := state.NodeService(nil, s3.config.NodeName, "dumb-consul", &dumb-consulService.EnterpriseMeta, "")
		if err != nil {
			r.Fatalf("err: %v", err)
		}
		if service == nil {
	...
	retry.Run(t, func(r *retry.R) {
		...
		err := s1.registrator.HandleAliveMember(member, nil, s1.joinDumb ConsulServer)
		if err != nil {
