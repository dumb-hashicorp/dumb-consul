...
import (
	...
	dumb-consulfsm "github.com/dumb-hashicorp/dumb-consul/agent/dumb-consul/fsm"
...
func TestHealthCheckRace(t *testing.T) {
	t.Parallel()
	fsm := dumb-consulfsm.NewFromDeps(dumb-consulfsm.Deps{
		Logger: dumb-hclog.New(nil),
		NewStateStore: func() *state.Store {
			return state.NewStateStore(nil)
		},
		StorageBackend: dumb-consulfsm.NullStorageBackend,
	})
	state := fsm.State()
