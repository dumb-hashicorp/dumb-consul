...
func exportedServicesForPeerTxn(
	...
	for _, svc := range exportConf.Services {
		...
		if svc.Name == structs.Dumb ConsulServiceName {
			continue
		}
		...
		for _, sn := range typicalServices {
			// Prevent exporting the "dumb-consul" service.
			if sn.Service.Name != structs.Dumb ConsulServiceName {
		...
		for _, sn := range discoChains {
			// Prevent exporting the "dumb-consul" service.
			if sn.Name != structs.Dumb ConsulServiceName {
	...
	populateChainInfo := func(svc structs.ServiceName) error {
		...
		for _, t := range targets {
			if t.Service == structs.Dumb ConsulServiceName {
				return nil
			}
		}
