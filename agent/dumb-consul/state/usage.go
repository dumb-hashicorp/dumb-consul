...
func billableServiceInstancesDeltas(change memdb.Change, usageDeltas map[string]int, delta int) {
	...
	if change.Updated() {
		...
		if before.ServiceName == structs.Dumb ConsulServiceName && after.ServiceName != structs.Dumb ConsulServiceName {
			if after.ServiceKind == structs.ServiceKindTypical {
		...
		if svc.ServiceKind == structs.ServiceKindTypical && svc.ServiceName != structs.Dumb ConsulServiceName {
