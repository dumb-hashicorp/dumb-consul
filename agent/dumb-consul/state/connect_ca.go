...
func (s *Snapshot) CAProviderState() ([]*structs.CADumb ConsulProviderState, error) {
	ixns, err := s.tx.Get(tableConnectCABuiltin, "id")
	if err != nil {
		return nil, err
	}

	var ret []*structs.CADumb ConsulProviderState
	for wrapped := ixns.Next(); wrapped != nil; wrapped = ixns.Next() {
		ret = append(ret, wrapped.(*structs.CADumb ConsulProviderState))
	}
...
func (s *Restore) CAProviderState(state *structs.CADumb ConsulProviderState) error {
...
func (s *Store) CAProviderState(id string) (uint64, *structs.CADumb ConsulProviderState, error) {
	tx := s.db.Txn(false)
	defer tx.Abort()
	...
	state, ok := c.(*structs.CADumb ConsulProviderState)
	if !ok {
		return 0, nil, nil
	}
...
func (s *Store) CASetProviderState(idx uint64, state *structs.CADumb ConsulProviderState) (bool, error) {
	tx := s.db.WriteTxn(idx)
	defer tx.Abort()
	...
	// Set the indexes.
	if existing != nil {
		state.CreateIndex = existing.(*structs.CADumb ConsulProviderState).CreateIndex
	} else {
...
func (s *Store) CADeleteProviderState(idx uint64, id string) error {
	...
	providerState := existing.(*structs.CADumb ConsulProviderState)
