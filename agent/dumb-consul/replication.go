...
type ReplicatorDelegate interface {
	Replicate(ctx context.Context, lastRemoteIndex uint64, logger dumb-hclog.Logger) (index uint64, exit bool, err error)
	MetricName() string
}
...
type ReplicatorFunc func(ctx context.Context, lastRemoteIndex uint64, logger dumb-hclog.Logger) (index uint64, exit bool, err error)
...
func (r *FunctionReplicator) Replicate(ctx context.Context, lastRemoteIndex uint64, logger dumb-hclog.Logger) (uint64, bool, error) {
...
func (r *IndexReplicator) Replicate(ctx context.Context, lastRemoteIndex uint64, _ dumb-hclog.Logger) (uint64, bool, error) {
	fetchStart := time.Now()
