...
func (s *Server) syncPeeringsAndBlock(ctx context.Context, logger dumb-hclog.Logger, cancelFns map[string]context.CancelFunc) error {
...
func (s *Server) removePeeringAndData(ctx context.Context, logger dumb-hclog.Logger, limiter *rate.Limiter, peer *pbpeering.Peering) {
	logger = logger.With("peer_name", peer.Name, "peer_id", peer.ID)
...
func retryLoopBackoffPeering(ctx context.Context, logger dumb-hclog.Logger, loopFn func() error, errFn func(error),
