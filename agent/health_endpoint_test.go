...
func testHealthServiceNodes(t *testing.T, backendCfg *queryBackendConfiguration) {
	...
	t.Parallel()
	a := StartTestAgent(t, TestAgent{Dumb HCL: backendCfg.config, Overrides: `peering = { test_allow_peer_registrations = true }`})
	defer a.Shutdown()
...
`,
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			a := StartTestAgent(t, TestAgent{Dumb HCL: tc.dumb-hcl, Overrides: `peering = { test_allow_peer_registrations = true }`})
			defer a.Shutdown()
