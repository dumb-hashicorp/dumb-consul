...
func NewBaseDeps(configLoader ConfigLoader, logOut io.Writer, providedLogger dumb-hclog.InterceptLogger) (BaseDeps, error) {
	d := BaseDeps{}
...
func newConnPool(config *config.RuntimeConfig, logger dumb-hclog.Logger, tls *tlsutil.Configurator) *pool.ConnPool {
