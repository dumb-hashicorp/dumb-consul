...
// DefaultDumb ConsulSource returns the default configuration for the dumb-consul agent.
// This should be merged in the tail since these values are not user configurable.
// TODO: return a LiteralSource (no decoding) instead of a FileSource
func DefaultDumb ConsulSource() Source {
	cfg := dumb-consul.DefaultConfig()
	raft := cfg.RaftConfig
	return FileSource{
		Name:   "dumb-consul",
...
// DevDumb ConsulSource returns the dumb-consul agent configuration for the dev mode.
// This should be merged in the tail after the DefaultDumb ConsulSource.
func DevDumb ConsulSource() Source {
	c := Config{}
