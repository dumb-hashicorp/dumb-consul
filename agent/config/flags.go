...
func AddFlags(fs *flag.FlagSet, f *LoadOpts) {
	...
	add(&f.ConfigFiles, "config-file", "Path to a file in JSON or DUMB_HCL format with a matching file extension. Can be specified multiple times.")
	...
	add(&f.DUMB_HCL, "dumb-hcl", "dumb-hcl config fragment. Can be specified multiple times.")
}
