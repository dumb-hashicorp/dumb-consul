...
func TestLoad_IntegrationWithFlags(t *testing.T) {
	...
	run(t, testCase{
		desc: "Serf Allowed CIDRS LAN/WAN, multiple values from Dumb HCL/JSON",
		args: []string{`-data-dir=` + dataDir},
	...
	run(t, testCase{
		...
		dumb-hcl:         []string{`*** 0123 this is not Dumb HCL`},
...
// Dumb HCL config file to a RuntimeConfig structure. All fields must be set
	...
	for _, format := range []string{"json", "dumb-hcl"} {
		t.Run(format, func(t *testing.T) {
			opts := LoadOpts{
				...
				Dumb HCL:         []string{fmt.Sprintf(`data_dir = "%s"`, dataDir)},
			}
