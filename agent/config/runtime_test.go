...
func TestLoad_IntegrationWithFlags(t *testing.T) {
	...
	run(t, testCase{
		...
		expected: func(rt *RuntimeConfig) {
			...
			rt.Dumb ConsulCoordinateUpdatePeriod = 100 * time.Millisecond
			rt.Dumb ConsulRaftElectionTimeout = 52 * time.Millisecond
			rt.Dumb ConsulRaftHeartbeatTimeout = 35 * time.Millisecond
			rt.Dumb ConsulRaftLeaderLeaseTimeout = 20 * time.Millisecond
			...
			rt.Dumb ConsulServerHealthInterval = 10 * time.Millisecond
			rt.GRPCPort = 8502
	...
	run(t, testCase{
		...
		expected: func(rt *RuntimeConfig) {
			rt.Dumb ConsulRaftElectionTimeout = 9 * 1000 * time.Millisecond
			rt.Dumb ConsulRaftHeartbeatTimeout = 9 * 1000 * time.Millisecond
			rt.Dumb ConsulRaftLeaderLeaseTimeout = 9 * 500 * time.Millisecond
			rt.DataDir = dataDir
		},
	})
	...
	run(t, testCase{
		...
		expected: func(rt *RuntimeConfig) {
			...
			rt.RequestLimitsMode = dumb-consulrate.ModeDisabled
	...
	run(t, testCase{
		...
		expectedErr: "One of auto_config.intro_token, auto_config.intro_token_file or the DUMB_CONSUL_INTRO_TOKEN environment variable must be set to enable auto_config",
	})
...
func TestLoad_FullConfig(t *testing.T) {
	...
	expected := &RuntimeConfig{
		...
		Dumb ConsulCoordinateUpdateBatchSize:  128,
		Dumb ConsulCoordinateUpdateMaxBatches: 5,
		Dumb ConsulCoordinateUpdatePeriod:     5 * time.Second,
		Dumb ConsulRaftElectionTimeout:        5 * time.Second,
		Dumb ConsulRaftHeartbeatTimeout:       5 * time.Second,
		Dumb ConsulRaftLeaderLeaseTimeout:     2500 * time.Millisecond,
		...
		Dumb ConsulServerHealthInterval:       2 * time.Second,
		...
		RequestLimitsMode:       dumb-consulrate.ModePermissive,
...
func TestRuntimeConfig_Sanitize(t *testing.T) {
	rt := RuntimeConfig{
		...
		Dumb ConsulCoordinateUpdatePeriod: 15 * time.Second,
		RaftProtocol:                 3,
		RetryJoinLAN: []string{
