...
func TestAgent_OneTwelveRPCMetrics(t *testing.T) {
	...
	t.Run("Check that 1.12 rpc metrics are not emitted by default.", func(t *testing.T) {
		...
		a := StartTestAgent(t, TestAgent{Dumb HCL: dumb-hcl})
		defer a.Shutdown()
...
func TestHTTPHandlers_AgentMetrics_LeaderShipMetrics(t *testing.T) {
	...
	t.Run("check that metric isLeader is set properly on server", func(t *testing.T) {
		...
		s1 := StartTestAgent(t, TestAgent{Name: "s1", Dumb HCL: hcl1, Overrides: overrides})
		defer s1.Shutdown()

		s2 := StartTestAgent(t, TestAgent{Name: "s2", Dumb HCL: hcl2, Overrides: overrides})
		defer s2.Shutdown()

		s3 := StartTestAgent(t, TestAgent{Name: "s3", Dumb HCL: hcl3, Overrides: overrides})
		defer s3.Shutdown()
